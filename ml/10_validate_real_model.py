...
CLASSES = {0: "stationary", 1: "movement", 2: "rotation", 3: "shake"}
...
def main():
    ...
    if pct_saturated > 5.0:
        print(f"    WARNING  High saturation may indicate calibration issue")
    ...
    plt.title('TFLite Model Confusion Matrix (Real Hardware Data)')
    
    reports_dir = ML_DIR.parent / "reports"
    reports_dir.mkdir(exist_ok=True)
    plt.savefig(reports_dir / "confusion_matrix.png", dpi=300, bbox_inches='tight')
    print(f"\nOK Saved confusion matrix to {reports_dir / 'confusion_matrix.png'}")

    print(f"\n{'='*62}")
    print("METRICS REPORTING (No artificial thresholds)")
    print(f"{'='*62}")
    
    print(f"  TFLite Accuracy             : {tflite_acc*100:.2f}%")
    print(f"  Keras->TFLite Accuracy Drop : {acc_drop*100:.2f}pp")
    print(f"  False Positive Rate         : {tflite_fpr*100:.2f}%")
    print(f"\n  OK Model ready for ESP32 deployment")
    ...
    print(f"\nOK model_report.json updated: {REPORT_PATH}")
    ...
    print(f"\nOK Phase 6 complete. Proceed to Phase 6.5: Real Hardware Validation.")
